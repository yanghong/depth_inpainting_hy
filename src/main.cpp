...
int main(int argc, char ** argv)
{
  ...
  if( instruction == "TVPHI")
    ...
    {
	  // depthImage missingRate outputMask outputMissing
      string disparityPath = argv[2];
      int missingRate = atof(argv[3]);//atof function??
  ...
  // for stat
    ...
    else if( instruction == "L0" )
    {
	  ...
	  // weight
      int W = dispU.cols;
	  // init a all zeros martrix
      Mat disp = Mat::zeros(H, W, CV_32FC1);
	  // change dispU to disp , type is CV_32FC1
	  // function converTo() is change matrix type
      dispU.convertTo(disp, CV_32FC1);
